...
static float chassis_gz = 0.0f;
static float gimbal_yaw_inter_input_chassis_gz_factor = -0.845;

#define IDEAL_6020_VOLTAGE 24.0f
static void compensate_6020_voltage(float *motor_out)
{
    *motor_out = *motor_out / get_battery_voltage() * IDEAL_6020_VOLTAGE;
    VAL_LIMIT(*motor_out, -30000.0f, +30000.0f);
}
...
int32_t gimbal_cascade_calculate(struct gimbal *gimbal)
{
    ...
    float chassis_yaw_rate = chassis_gz * RAD_TO_DEG;
    outer_out += gimbal_yaw_inter_input_chassis_gz_factor * chassis_yaw_rate;
    motor_out = pid_calculate(&(gimbal->yaw_inter_pid), gimbal->sensor.rate.yaw_rate, outer_out);
    compensate_6020_voltage(&motor_out);
...
int32_t gimbal_recv_chassis_imu(uint8_t *buf, uint16_t len) {
    if (len == sizeof(struct cmd_chassis_state)) {
        struct cmd_chassis_state *chassis_state = (struct cmd_chassis_state *) buf;
        chassis_gz = chassis_state->gz;
    }
