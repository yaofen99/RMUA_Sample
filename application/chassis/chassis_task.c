...
static void push_chassis_imu_to_gimbal(void);
...
static void push_chassis_imu_to_gimbal(void) {
    struct cmd_chassis_state chassis_state;
    chassis_state.gz = chassis_gyro.gz;
    protocol_send(GIMBAL_ADDRESS, CMD_PUSH_CHASSIS_STATE, &chassis_state, sizeof(struct cmd_chassis_state));
}
