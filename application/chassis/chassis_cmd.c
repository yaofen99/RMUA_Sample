...
extern struct ahrs_sensor chassis_gyro;
...
int32_t chassis_info_push(void *argc)
{
    ...
    struct cmd_imu_info imu_info;

    imu_info.gx = chassis_gyro.gx;
    imu_info.gy = chassis_gyro.gy;
    imu_info.gz = chassis_gyro.gz;
    imu_info.ax = chassis_gyro.ax;
    imu_info.ay = chassis_gyro.ay;
    imu_info.az = chassis_gyro.az;
    imu_info.qw = chassis_gyro.qw;
    imu_info.qx = chassis_gyro.qx;
    imu_info.qy = chassis_gyro.qy;
    imu_info.qz = chassis_gyro.qz;

    protocol_send(MANIFOLD2_ADDRESS, CMD_PUSH_CHASSIS_IMU, &imu_info, sizeof(imu_info));

    struct cmd_battery_info battery_info;
    battery_info.voltage = get_battery_voltage();
    protocol_send(MANIFOLD2_ADDRESS, CMD_PUSH_CHASSIS_BATTERY, &battery_info, sizeof(battery_info));
