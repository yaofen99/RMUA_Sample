...
extern struct ahrs_sensor gimbal_gyro;
...
int32_t gimbal_info_push(void *argc)
{
    ...
    imu_info.gx = gimbal_gyro.gx;
    imu_info.gy = gimbal_gyro.gy;
    imu_info.gz = gimbal_gyro.gz;
    imu_info.ax = gimbal_gyro.ax;
    imu_info.ay = gimbal_gyro.ay;
    imu_info.az = gimbal_gyro.az;
    imu_info.qw = gimbal_gyro.qw;
    imu_info.qx = gimbal_gyro.qx;
    imu_info.qy = gimbal_gyro.qy;
    imu_info.qz = gimbal_gyro.qz;

    protocol_send(MANIFOLD2_ADDRESS, CMD_PUSH_GIMBAL_IMU, &imu_info, sizeof(imu_info));

    struct cmd_battery_info battery_info;
    battery_info.voltage = get_battery_voltage();
    protocol_send(MANIFOLD2_ADDRESS, CMD_PUSH_GIMBAL_BATTERY, &battery_info, sizeof(battery_info));
